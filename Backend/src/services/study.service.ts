 ...
 */
function computeNextReview(streak: number): { status: CardStatus; nextReviewAt: Date } {
  if (streak >= 2) {
    return {
      status: "MASTERED",
...
export class StudyService {
  ...
  async updateSet(setId: string, input: UpdateSetInput) {
    const updated = await studyRepository.updateSet(setId, input).catch((err: any) => {
      if (err?.code === "P2025") throw new UpdatedError("Flashcard set not found", 404, err);
  ...
  async addCardsToSet(setId: string, cards: CreateCardInput[]) {
    const updated = await studyRepository.addCardsToSet(setId, cards).catch((err: any) => {
      if (err?.code === "P2025") throw new UpdatedError("Flashcard set not found", 404, err);
  ...
  async syncProgress(userId: string, sessionId: string) {
    ...
    const score =
      totalCards > 0 ? Math.round((sessionState.correctCount / totalCards) * 100) : 0;
