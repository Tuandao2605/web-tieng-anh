...
const INDEX_NAME =
  process.env.ELASTICSEARCH_DECK_INDEX ?? "public_flashcard_decks_v1";
...
export const elasticsearchService = {
  async searchPublicDecks(
    ...
    return {
      decks: response.hits.hits.flatMap((hit) =>
  ...
  async syncDeck(deckId: string, mutationVersion: number = Date.now()) {
    await ensureIndex();
    ...
    if (document) {
      await client
        .index({
          ...
          id: deckId,
          document,
          version: deck.updatedAt.getTime(),
          version_type: "external_gte",
        })
        .catch((error: unknown) => {
          // A newer job already won. Treat this stale job as successfully
          // superseded instead of retrying it and adding queue pressure.
          if (!isVersionConflict(error)) throw error;
        });
    }
  },
  ...
  async reindexPublicDecks() {
    ...
    do {
      ...
      if (response.errors)
        throw new Error("Elasticsearch bulk reindex contained failed items");
